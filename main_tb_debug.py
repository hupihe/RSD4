...
from env.scheduler_csi_multihop_ve import scheduler_csi_multihop_ve
...
from env.scheduler_data_multihop_ve import scheduler_data_multihop_ve
from env.scheduler_data_multihop_ve_op import scheduler_data_multihop_ve_op

def eval_policy(policy, eval_episodes=20):
    if args.env == 'scheduler_simple':
        eval_env = scheduler_simple(lamb=args.lamb, user_num=args.user_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2 + 100, partial=args.partial, homogeneous=args.homogeneous, period=args.period, cntype=args.cntype, square=args.square, poisson=args.poisson)
    elif args.env == 'scheduler_csi':
        eval_env = scheduler_csi(lamb=args.lamb, user_num=args.user_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2 + 100, partial=args.partial, homogeneous=args.homogeneous, period=args.period, cntype=args.cntype, square=args.square, poisson=args.poisson)
    elif args.env == 'scheduler_csi_multihop_ve':
        eval_env = scheduler_csi_multihop_ve(lamb=lamb_lst, user_num=args.user_num, hop_num=args.hop_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2+100, partial=args.partial, homogeneous=args.homogeneous, period=args.period, cntype=args.cntype, square=args.square, poisson=args.poisson)
    elif args.env == 'scheduler_mem':
        eval_env = scheduler_mem(lamb=args.lamb, user_num=args.user_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2 + 100, partial=args.partial, homogeneous=args.homogeneous, period=args.period, cntype=args.cntype, square=args.square, poisson=args.poisson)
    elif args.env == 'scheduler_data':
        eval_env = scheduler_data(arrival_data_path, channel_data_path, lamb=args.lamb, user_num=args.user_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2 + 100, partial=args.partial, homogeneous=args.homogeneous)
    elif args.env == 'scheduler_datasimple':
        eval_env = scheduler_datasimple(arrival_data_path, channel_data_path, lamb=args.lamb, user_num=args.user_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2 + 100, partial=args.partial, homogeneous=args.homogeneous)
    elif args.env == 'scheduler_data_multihop_ve':
        eval_env = scheduler_data_multihop_ve(arrival_data_path, channel_data_path, lamb=lamb_lst, user_num=args.user_num, hop_num=args.hop_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2 + 100, partial=args.partial)
    elif args.env == 'scheduler_data_multihop_ve_op':
        eval_env = scheduler_data_multihop_ve_op(arrival_data_path, channel_data_path, lamb=lamb_lst, user_num=args.user_num, hop_num=args.hop_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2 + 100, partial=args.partial)
    ...
    for episode_idx in range(eval_episodes):
        ...
        last_action = np.zeros(eval_env.action_dim) - 1
        hidden_out = (torch.zeros([args.num_layers, 1, hidden_dim], dtype=torch.float).to(device), torch.zeros([args.num_layers, 1, hidden_dim], dtype=torch.float).to(device))
        while not done:
            hidden_in = hidden_out
            action, hidden_out = policy.select_action(state, last_action, hidden_in)
            last_action = action
    ...
    #print(all_episode_energy)

    last_action = np.zeros(eval_env.action_dim) - 1
    hidden_in = (torch.zeros([args.num_layers, 1, hidden_dim], dtype=torch.float).to(device), torch.zeros([args.num_layers, 1, hidden_dim], dtype=torch.float).to(device))
    if args.partial:
        state = np.array([1] * env.action_dim)
    else:
        state = np.array([1] * env.action_dim + [0] * env.N)
    action, hidden_out = policy.select_action(state, last_action, hidden_in)
    ...
    mean_episode_energy = np.mean(all_episode_energy, axis=0)
    std_episode_energy = np.std(all_episode_energy)
    #print(mean_episode_energy)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambid", default=0, type=int)
    ...
    parser.add_argument("--hop-num", default=2, type=int)
    parser.add_argument("--taum", default=6, type=int)
    ...
    parser.add_argument("--update-itr", default=1, type=int)
    ...
    lamb_lst = [0] * args.hop_num
    lamb_lst[args.lambid] = args.lamb

    if args.env == 'scheduler_simple':
        env = scheduler_simple(lamb=args.lamb, user_num=args.user_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2, partial=args.partial, homogeneous=args.homogeneous, period=args.period, cntype=args.cntype, square=args.square, poisson=args.poisson)
    elif args.env == 'scheduler_csi':
        env = scheduler_csi(lamb=args.lamb, user_num=args.user_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2, partial=args.partial, homogeneous=args.homogeneous, period=args.period, cntype=args.cntype, square=args.square, poisson=args.poisson)
    elif args.env == 'scheduler_csi_multihop_ve':
        env = scheduler_csi_multihop_ve(lamb=lamb_lst, user_num=args.user_num, hop_num=args.hop_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2, partial=args.partial, homogeneous=args.homogeneous, period=args.period, cntype=args.cntype, square=args.square, poisson=args.poisson)
    elif args.env == 'scheduler_mem':
        env = scheduler_mem(lamb=args.lamb, user_num=args.user_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2, partial=args.partial, homogeneous=args.homogeneous, period=args.period, cntype=args.cntype, square=args.square, poisson=args.poisson)
    elif args.env == 'scheduler_data':
        env = scheduler_data(arrival_data_path, channel_data_path, lamb=args.lamb, user_num=args.user_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2, partial=args.partial, homogeneous=args.homogeneous)
    elif args.env == 'scheduler_datasimple':
        env = scheduler_datasimple(arrival_data_path, channel_data_path, lamb=args.lamb, user_num=args.user_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2, partial=args.partial, homogeneous=args.homogeneous)
    elif args.env == 'scheduler_data_multihop_ve':
        env = scheduler_data_multihop_ve(arrival_data_path, channel_data_path, lamb=lamb_lst, user_num=args.user_num, hop_num=args.hop_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2, partial=args.partial)
    elif args.env == 'scheduler_data_multihop_ve_op':
        env = scheduler_data_multihop_ve_op(arrival_data_path, channel_data_path, lamb=lamb_lst, user_num=args.user_num, hop_num=args.hop_num, tau_max=args.taum, time_step=args.train_len, seed1=args.seed1, seed2=args.seed2, partial=args.partial)
    ...
    for eps in range(int(args.episode_num)):
        state = env.reset()
        last_action = np.zeros(env.action_dim) - 1
        episode_state = []
        episode_action = []
        episode_last_action = []
        episode_reward = []
        ...
        for step in range(int(args.train_len)):
            ...
            if frame_idx < args.start_steps:
                ...
                action, hidden_out = policy.select_action(state, last_action, hidden_in)
                action = (action + np.random.normal(0, max_action * args.action_noise, size=action_dim)).clip(-max_action, max_action)
            ...
            #print('state: ', state, 'action: ', action, 'last_action: ', last_action, 'reward: ', reward, 'next_state: ', next_state, 'done: ', done)
            state = next_state
            last_action = action
            frame_idx += 1
        ...
        if eps % args.eval_freq == 0:
            ...
            if 've' in args.env:
                avg_energy = avg_energy[args.lambid]
        ...
        if 've' in args.env:
            df.loc[eps] = [eps, mean_reward, info['throughput'], info['energy'][args.lambid], avg_reward, avg_throughput, avg_energy]
            print(args.name, 'Episode: ', eps, "| Best reward: {:.2f} - Current reward: {:.2f} - Throughput: {:.2f} - Energy: {:.2f}".format(best_reward, mean_reward, info['throughput'], info['energy'][args.lambid]))
        else:
